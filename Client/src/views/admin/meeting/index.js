...
const Index = () => {
  ...
  const tableColumns = [
    ...
    { Header: "Time Stamp", accessor: "timestamp" },
    {
      ...
      accessor: "createBy",
      cell: (cell) => {
        const createBy = cell.value;
        if (!createBy) return "-";
        // If createBy is an object, show firstName and lastName
        if (
          typeof createBy === "object" &&
          (createBy.firstName || createBy.lastName)
        ) {
          return `${createBy.firstName ?? ""} ${
            createBy.lastName ?? ""
          }`.trim();
        }
  ...
  const fetchData = async () => {
    ...
    if (result.payload.status === 200) {
      console.log(result?.payload?.data?.meetings);
